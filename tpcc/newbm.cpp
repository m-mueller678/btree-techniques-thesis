...
void initWorkerId() { if (workerThreadId) return; workerThreadId = workerThreadIdCounter++; assert(workerThreadId<maxWorkerThreads); }
...
struct LibaioInterface {
   ...
   void writePages(vector<PID>& pages) {
...
struct BufferManager {
   ...
   u64 ppCount;
   atomic<bool> ppRun;
   vector<thread> ppThreads;

   PageState& getPageState(PID pid) {
#ifdef MMAPFILE
      return virtMem[pid].ps;
#else
...
BufferManager::BufferManager() : virtSize(envOr("VIRTGB", 16)*gb), physSize(envOr("PHYSGB", 4)*gb), virtCount(virtSize / pageSize), physCount(physSize / pageSize), residentSet(physCount) {
   ...
   ppCount = envOr("PPCOUNT", 0);
   ppRun = true;
   for (u64 i=0; i<ppCount; i++) {
      ppThreads.emplace_back([&]() {
         initWorkerId();
         while (ppRun.load()) {
...
#ifndef MMAPFILE
   if (ppCount) {
      while (physUsedCount >= physCount*0.98)
         yield(1);
      return;
   }
   if (physUsedCount >= physCount*0.95)
...
struct BTreeNode : public BTreeNodeHeader {
   ...
   bool mergeNodes(unsigned slotId, BTreeNode* parent, BTreeNode* right)
   {
      if (isLeaf) {
         ...
         unsigned leftGrow = (prefixLen - tmp.prefixLen) * count;
         unsigned rightGrow = (right->prefixLen - tmp.prefixLen) * right->count;
         unsigned extraKeyLen = parent->prefixLen + parent->slot[slotId].keyLen;
         unsigned spaceUpperBound = spaceUsed + right->spaceUsed + (reinterpret_cast<u8*>(slot + count + right->count) - ptr()) + leftGrow + rightGrow + tmp.spaceNeeded(extraKeyLen, sizeof(PID));
         if (spaceUpperBound > pageSize)
            return false;
         copyKeyValueRange(&tmp, 0, 0, count);
         u8 extraKey[extraKeyLen];
         memcpy(extraKey, parent->getLowerFence().data(), parent->prefixLen);
         memcpy(extraKey+parent->prefixLen, parent->getKey(slotId), parent->slot[slotId].keyLen);
         storeKeyValue(count, {extraKey, extraKeyLen}, parent->getPayload(slotId));
         count++;
         right->copyKeyValueRange(&tmp, count, 0, right->count);
         parent->removeSlot(slotId);
